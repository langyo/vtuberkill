...
function hexagonal(
  ...
  for (let a = angle[0]; a <= angle[1]; a++) {
    line.push([
      center[0] + dist * Math.sin(a * dig),
      center[1] - dist * Math.cos(a * dig),
    ]);
  }
  ...
  for (let drct of drcts) {
    ...
    if (serif) {
      AddLine(
        line,
        drct + 1.25,
        (dist * Math.cos(0.5 * dig)) / Math.cos(0.25 * dig)
      );
      AddLine(
        line,
        drct - 0.25,
...
class City extends Area {
  ...
  drawBorder(
    ...
    for (let b of blocks) {
      let pos = b.pos;
      if (b.inSize) {
        let direction = b.getAdjoinBlock(
          (x, y) => x.cityId !== y.cityId
        ).direction;
        if (direction.length) {
    ...
    ctx.fillStyle = c.color
      ? hexToRgba(c.color, opacity, [-20, -20, -20])
      : `rgba(150, 50, 255, ${opacity})`;
    ctx.fill();
    ctx.beginPath();
    ctx.restore();
  }
  addBlock(x: number, y: number) {
    ...
    this.blocks.push(
      new Block([this.pos[0] + x / 2 + y / 4, this.pos[1] + y / 2], {
    ...
    if (this.inSize()) {
      ...
      if (config.hiddenName !== true) {
        c.drawName(
          ...
          size,
          zoom,
          config.hiddenName > 0 ? config.hiddenName : 0.85
        );
      }
    } else {
      return false;
    ...
    for (let b of blocks) {
      ...
      for (let v in adjoins) {
        if (adjoins[v] instanceof Array) {
          if (
            bpos[0] - pos[0] === adjoins[v][0] &&
            bpos[1] - pos[1] === adjoins[v][1]
          ) {
            adjoins[v] = b;
        ...
        switch (data.map_status) {
          case "城市":
            ctx.fillStyle = area.color
              ? hexToRgba(area.color, opacity)
              : `rgba(150, 50, 255, ${opacity})`;
            break;
          case "势力":
          case "外交":
            ctx.fillStyle = area.faction
              ? hexToRgba(area.faction.color, opacity)
              : `rgba(55, 55, 65, 0.4)`;
            break;
          case "省区":
            ctx.fillStyle = area.region
              ? hexToRgba(area.region.color, opacity)
              : `rgba(150, 50, 255, ${opacity})`;
            break;
        }
      } else {
...
class Faction extends Areas {
  ...
  constructor(
    name: string,
    citynames: string[],
    info: { chara?: anyObject; config: anyObject }
  ) {
    super(name);
  ...
  MapHighlight(
    ...
    for (let b of blocks) {
      ...
      if (b.inSize) {
        let direction = b.getAdjoinBlock(
          (x, y) => x.area?.faction === y.area?.faction
        ).reserve;
        if (direction.length) {
          let coord = [(pos[0] - size[0][0]) * 2, (pos[1] - size[0][1]) * 2];
          hexAnnulus(ctx, coord, zoom, direction);
...
class Resource {
  ...
  constructor(
    name: string,
    allocat: Record<
      City["name"] | Faction["name"],
      [number] | [number, number]
    >,
    info: anyObject = { reserves: {} }
  ) {
    this.name = name;
    ...
    this.allocation = allocat as Record<
      City["name"] | Faction["name"],
      [number, number]
    >;
  ...
  factionHarvest(factionName: Faction["name"]) {
    ...
    if (context.tempCtx) {
      curf.showInformation(context.tempCtx, context.curSize, context.curZoom, {
        text: `${translation.resource[this.name]}${this.abbreviation}${
          harvest > 0 ? "增加" : "减少"
        }了${harvest}`,
      });
      if (data.timeRecord) {
...
const citys = [
  ...
  new City("wan", [12, 14], {
    level: 4,
    color: "#f3d48a",
    shape: [-3, [0, 1], [-2, 2], [-2, 2], [-1, 2], [-2, 2], [-2, 1], [-1, 0]],
  }),
  new City("shangyong", [10, 15], {
    level: 2,
    color: "#94d685",
    shape: [-2, [-1, 1], [-2, 1], [-1, 2], [-1, 3], [0, 3], 1],
  }),
  ...
  new City("guangling", [23, 15], {
    level: 4,
    color: "#6fa1eb",
    shape: [-3, [2, 5], [-2, 5], [-3, 5], [-3, 3], [-3, -1], -3],
  }),

  new City("shouchun", [19, 15], {
    level: 5,
    color: "#f7b063",
    shape: [-3, [0, 4], [-1, 4], [-2, 4], [-3, 4], [-2, 4], [-1, 4], [-1, 2]],
  }),
  new City("lujiang", [21, 18], {
    level: 2,
    color: "#e0d57a",
    shape: [-3, [2, 3], [-1, 3], [-2, 2], [-4, 2], [-4, 1], [-4, -1]],
  }),

  new City("xuchang", [15, 14], {
    level: 4,
    color: "#7ec7ff",
    shape: [-2, [-1, 1], [-2, 3], [-3, 5], [-3, 4], [-4, 3], [-2, 0]],
  }),
  new City("runan", [17, 16], {
    level: 3,
    color: "#8198fc",
    shape: [-1, [-1, 2], [-3, 3], [-2, 3], [0, 3]],
  }),

  new City("xinye", [14, 17], {
    level: 2,
    color: "#ea9370",
    shape: [-3, [0, 2], [-1, 3], [-2, 4], [-3, 5], [-1, 2]],
  }),
  new City("xiangyang", [13, 19], {
    level: 6,
    color: "#a893c8",
    shape: [-4, -4, [-5, -2], [-4, -1], [-3, 1], [-3, 4], [-2, 4], [0, 4]],
  }),
  new City("jiangxia", [17, 19], {
    level: 4,
    color: "#8ecdb7",
    shape: [-3, [-1, 1], [-1, 4], [-1, 4], [-1, 4], [0, 3]],
  }),
  new City("jiangling", [14, 22], {
    level: 3,
    color: "#6a84b7",
    shape: [-4, [-1, 0], [-2, 5], [-3, 4], [-2, 4], [-1, 3], [0, 1]],
  }),

  new City("changsha", [16, 24], {
    level: 4,
    color: "#c4e759",
    shape: [-2, [1, 3], [-1, 3], [-3, 4], [-4, 3], [-4, 3], [1, 2]],
  }),
...
const regions = [
  ...
  new Region(
    "扬州",
    ["jianye", "wu", "chaisang", "kuaiji", "jianan", "luling"],
    {
      config: {
        color: "#bbd97b",
      },
...
const control = {
  ...
  tempSynchronization() {
    [context.tempCvs.width, context.tempCvs.height] = [
      context.curCvs.width,
      context.curCvs.height,
    ];
  },
  tempClear() {
